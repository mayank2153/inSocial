    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          { vote, post },
          "Vote has been successfully created",
        ),
      );
  } catch (error) {
    return res
      .status(500)
      .json(new ApiError(500, `Error creating vote: ${error.message}`));
  }
});
...
const deleteVote = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, null, "Vote deleted successfully"));
  } catch (error) {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          {
            upvotes: { count: upvotesCount, users: upvotes },
            downvotes: { count: downvotesCount, users: downvotes },
          },
          "Votes have been successfully fetched",
        ),
      );
  } catch (error) {
...
export { createVote, deleteVote, getVotesByPost };
