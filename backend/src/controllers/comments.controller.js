...
const createComment = asyncHandler(async (req, res) => {
  ...
  if (parentCommentId) {
    ...
    if (!parentComment) {
      throw new ApiError(
        404,
        "Parent comment not found or there was an error in finding the parent comment",
      );
    }
...
const getCommentsByPost = asyncHandler(async (req, res) => {
  try {
    ...
    const comments = await Comment.find({ post: postId, parentCommentId: null })
      .populate("owner")
      .exec();
...
export { createComment, deleteComment, getCommentsByPost, getCommentById };
