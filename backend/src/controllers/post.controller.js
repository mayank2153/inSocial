...
const createNewPost = asyncHandler(async (req, res) => {
  ...
  const cloudinaryMedia = mediapostLocalPath
    ? await uploadOnCloudinary(mediapostLocalPath)
    : null;
...
const updatePost = asyncHandler(async (req, res) => {
  ...
  // Check valid input
  if (
    [title, description, category].some((field) => field && field.trim() === "")
  ) {
    throw new ApiError(400, "Please provide necessary details");
  }
  ...
  if (!post) {
    throw new ApiError(
      404,
      "Post not found or there was an error in finding the post",
    );
  }
...
const deletePost = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, null, "Post has been successfully deleted"));
});
...
const getPostsByUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, posts, "Post has been successfully fetched"));
});
...
const fetchPosts = asyncHandler(async (req, res) => {
  ...
  if (user.likedCategories && user.likedCategories.length > 0) {
    ...
    posts = await Post.find({ category: { $in: user.likedCategories } }).sort({
      createdAt: -1,
    });
  } else {
    // Fetch all posts
