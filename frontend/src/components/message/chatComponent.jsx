  ...
  const CustomMessage = ({ content, date, isSent }) => {
    return (
      <div className={`flex ${isSent ? 'justify-end' : 'justify-start'}`}>
        <div
          className={`max-w-[70%] p-3 rounded-lg ${isSent ? 'bg-blue-500 text-white' : 'bg-gray-200 text-black'} shadow-lg mb-2 overflow-x-clip`}
        >
  ...
  return (
    <div className="max-h-[85vh] w-full flex flex-col px-1">
      ...
      <div className="w-full py-4 text-md bg-slate-600 lg:max-w-[300px] rounded-2xl flex justify-between px-2 mx-1 fixed bottom-0">
        ...
        />
        <button onClick={sendMessage} className="px-2 text-blue-500">
          <IoSend size={25} />
        </button>
      </div>
    </div>
