    ...
    const fetchResults = async () => {
      try {
        const response = await axios.get(
          `${url}search/searchUsersAndPosts?query=${query}`,
          {
  ...
  return (
    <div className="w-full h-[100vh] overflow-y-scroll no-scrollbar flex bg-[#0d1114] flex-col  py-4 ">
      <div className=" lg:min-w-[500px] min-w-[350px]  h-[500px] bg-[#0d1114] py-2 px-8 flex flex-col gap-4">
        <div
          className="flex text-white justify-start  text-xl font-mono border-b border-gray-600
            "
        >
          <div
            className={`border-r px-2 hover:bg-[#2e2b2b] py-1
          ...
          <div
            className={`px-2 hover:bg-[#2e2b2b] py-1
