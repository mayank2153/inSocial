...
const Previews = ({ setFormData, initialMedia }) => {
  ...
  const thumbs = files.map((file) => (
    <div
      className="relative inline-flex rounded border border-gray-300 mb-2 mr-2 p-1 box-border min-w-max"
      key={file.name || file.url || file}
    >
      <button
  ...
  return (
    <section className="container h-max min-w-max">
      <div
        ...
        className={`p-4 rounded cursor-pointer w-full ${files.length ? 'hidden' : ''}`}
      >
      ...
      <aside className="flex flex-wrap mt-4 w-full">{thumbs}</aside>
    </section>
  );
};
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = isEdit
        ...
        : await axios.post(`${url}posts/create-post`, data, {
            withCredentials: true,
          });
      navigate('/');
      setLoading(false);
      ...
      <div className="flex flex-col items-center bg-[#0d1114] rounded-2xl max-h-[100vh] text-white px-6 py-5 md:px-10 md:py-8 gap-4 w-full max-w-[600px]">
        ...
        <div>
          <form
            ...
            className="flex flex-col text-lg md:text-2xl gap-6 md:gap-8 w-full"
          >
            <div className="lg:max-h-20 overflow-auto no-scrollbar max-h-20">
              ...
              >
                <option value="" disabled className="bg-[#13181d]">
                  Select Category
                </option>
