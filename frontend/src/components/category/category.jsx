...
const ShowCategories = ({ hideCategories }) => {
    ...
    return isAuthenticated ? (
        <div className="bg-[#0d1114] h-screen shadow-xl overflow-y-scroll no-scrollbar border-r border-gray-600 w-full">
            ...
            <div className="mt-2 border-b-2 border-slate-500 pb-4 mb-10">
                <h1 className="text pl-20 text-slate-400">Categories</h1>
                <ul className="mt-3 pl-20">
                    {categories.map((category) => (
                        <Link to={`/posts/category/${category._id}`} key={category._id} onClick={hideCategories}>
                            <div className="flex gap-2 mb-4 ml-4">
                                <li className="rounded-full">
                                    <img className="rounded-full h-8 w-8 object-cover" src={category.media} alt={category.name} />
                                </li>
                                <li className="mt-1 text-sm text-slate-100">{category.name}</li>
                            </div>
                        </Link>
                    ))}
                </ul>
