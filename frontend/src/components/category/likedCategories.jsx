...
const LikedCategories = () => {
  ...
  const fetchOwner = async () => {
    try {
      ...
      console.error(
        'There seems to be an error while fetching owner details',
        error,
      );
    }
  };
  ...
  const likedCategories_redux = useSelector(
    (state) => state.likedCategories.likesCategory,
  );

  const fetchingCategoriesByid = async () => {
    try {
      const categoryIds = likedCategories_redux.map(
  ...
  const handleRemoveCategory = async (categoryId) => {
    ...
    if (confirmRemove) {
      try {
        await axios.post(
          `${url}users/remove-liked-category`,
          {
            ownerID,
            categoryId,
          },
        ...
        setCategories((prevCategories) =>
          prevCategories.filter((category) => category.data._id !== categoryId),
        );
  ...
  if (
    likedCategories_redux &&
    categories.length < likedCategories_redux.length
  ) {
    return <LikedCategoriesShimmer />;
  }
