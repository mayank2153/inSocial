...
const PostCard = ({
  ...
  const handleVote = async (voteType) => {
    if (userVote) {
      ...
      const voteId = votes.find(
        (vote) => vote.voteOwner === currentUser,
      )?.voteId;
  ...
  const removeVote = async (voteId) => {
    try {
      const deleteVote = await axios.post(
        `${url}vote/delete-vote/${voteId}`,
        {},
        { withCredentials: true },
      );
    } catch (error) {
  ...
  return (
    <div
      className="post-card  rounded-lg py-1  w-full lg:max-w-[650px] min-w-[300px] lg:min-w-[650px]"
      ...
      >
        <div className="flex sm:flex-row gap-4 sm:gap-6 justify-between">
          ...
          <div className="flex-1">
            <p className="text-sm text-white flex justify-end mr-0 lg:mr-4">
            ...
            <div className="media mt-4 rounded-2xl border border-slate-200 flex justify-center">
              <a href={media} target="_blank" rel="noopener noreferrer">
                <img
                  src={media}
                  alt="Media content"
                  className="max-w-full max-h-[220px] object-cover"
                />
              </a>
            </div>
          )}
        ...
        <div className="text-sm text-gray-500 flex items-center sm:flex-row gap-4 mt-2">
          <div className="flex items-center gap-2">
            <div
              className={`flex rounded-full gap-1 cursor-pointer
