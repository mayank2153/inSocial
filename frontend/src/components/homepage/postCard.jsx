      ...
      try {
        const [ownerResponse, categoryResponse] = await Promise.all([
          axios.get(`${url}users/get-user/${owner}`, {
            ...
            withCredentials: true  // Make sure this is necessary based on your CORS setup
          }),
        ...
        setOwnerDetails(ownerResponse.data.data); // Assuming response.data.data contains user details
        setCategoryDetails(categoryResponse.data.data.name); // Assuming response.data.name contains category name
      } catch (error) {
